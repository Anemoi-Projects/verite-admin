...
const Privacy = () => {
  ...
  const getAllBlogs = () => {
    let config = {
      method: "get",
      maxBodyLength: Infinity,
      url: `${process.env.apiURL}/api/v1/contents/getFooterPage?slug=/privacy-policy&lang=${region}`,
      headers: {},
    };
        ...
        <>
          <div className="flex  items-center justify-end gap-x-4">
            <Pencil
              ...
              onClick={() => {
                router.push(
                  `/legal-disclosure/privacy-policy/${row.original._id}?language=${region}`
                );
              }}
            />
            {/* <Trash2
