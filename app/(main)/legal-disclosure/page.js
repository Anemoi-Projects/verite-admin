...
const Page = () => {
  ...
  const getAllBlogs = () => {
    let config = {
      method: "get",
      maxBodyLength: Infinity,
      url: `${process.env.apiURL}/api/v1/contents/getFooterPage?slug=/legal-disclosure&lang=${region}`,
      headers: {},
    };
        ...
        <>
          <div className="flex  items-center justify-end gap-x-4">
            <Pencil
              ...
              onClick={() => {
                router.push(
                  `/legal-disclosure/${row.original._id}?language=${region}`
                );
              }}
            />
            {/* <Trash2
