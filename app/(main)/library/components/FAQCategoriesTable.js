...
const FAQCategoriesTable = ({
  ...
  const getAllFAQCategories = (token) => {
    let config = {
      method: "get",
      maxBodyLength: Infinity,
      url:
        `${process.env.apiURL}/api/v1/admin/getAllCategories?slug=faq&lang=` +
        selectedLanguage,
  ...
  useEffect(() => {
    if (window.localStorage.getItem("authToken")) {
      ...
      getAllFAQCategories(localStorage.getItem("authToken"));
    }
  }, [selectedLanguage]);
  const deleteCategory = () => {
    ...
    axios
      ...
      .catch((error) => {
        ...
        setShowDeleteModal(false);
        toast.error("Error while deleting FAQ Category");
        console.log(error);
      });
  };
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 max-h-[90vh] w-full max-w-lg -translate-x-1/2 -translate-y-1/2 transform rounded-lg bg-white p-6 shadow-lg z-50 overflow-y-auto transition-all animate-in fade-in-90">
            <div className="flex justify-between items-center mb-4 border-b-4 pb-2 border-[#140B49]">
              <div className="flex justify-center items-center gap-2">
                ...
                <Dialog.Title className="text-lg font-semibold text-black">
                  {blogCategoryForm?.state?.toUpperCase()} FAQ Category
                </Dialog.Title>
              </div>
            ...
            <FAQCategoryForm
              ...
              getAllBlogCategories={getAllFAQCategories}
