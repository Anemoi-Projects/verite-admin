  ...
  useEffect(() => {
    if (state === "add") {
      ...
      axios
        ...
        .catch((error) => {
          console.error("Failed to fetch Blog Category data", error);
        });
    }
  }, [state, id]);
  // comment
  const handleDraftSave = () => {
    ...
    if (name) {
      ...
      axios
        ...
        .then((response) => {
          toast.success(
            `Blog Category ${isAddMode ? "created" : "updated"} successfully!`
          );
          getAllBlogCategories();
          setShowBlogCategoryPanel(false);
        ...
        .catch((err) => {
          toast("Failed to save Blog Category");
        });
    } else {
      toast(
        "Please fill the mandatory fields (name) before saving the Blog Category"
      );
    }
  };
  useEffect(() => {
