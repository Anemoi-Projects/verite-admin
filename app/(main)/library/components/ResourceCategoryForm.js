  ...
  useEffect(() => {
    if (state === "add") {
      ...
      axios
        ...
        .catch((error) => {
          console.error("Failed to fetch Resource Category data", error);
        });
    }
  }, [state, id]);
  // comment
  const handleDraftSave = () => {
    ...
    if (name) {
      ...
      axios
        ...
        .catch((err) => {
          toast("Failed to save Resource Category");
        });
    } else {
      toast(
        "Please fill the mandatory fields (name) before saving the Resource Category"
      );
    }
  };
  useEffect(() => {
